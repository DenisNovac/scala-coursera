...
object Server extends ServerModuleInterface {
  ...
  val identityParserSink: Sink[ByteString, Future[Identity]] = {
    val sink: Sink[Identity, Future[Identity]] = Sink.head
    Flow[ByteString]
      .via(reframedFlow)
      .map(Identity.parse)
      .toMat(sink)(Keep.right)
    //.toMat(sink)((notUsed, m) => m)  // То же самое
  }
  ...
  val reintroduceOrdering: Flow[Event, Event, NotUsed] =
    Flow[Event].statefulMapConcat { () =>
      var buff: Map[Int, Event] = Map()
      var lastProduced: Int = 0

      def sorted: List[Event] = buff.toList.sortWith(_._1 < _._1).map(t => t._2)
      def isNext(e: Event): Boolean = e.sequenceNr == (lastProduced + 1)

      event =>
        {
          // нашли следующий элемент
          if (isNext(event)) {
            buff += event.sequenceNr -> event
            val r = sorted
            lastProduced = r.last.sequenceNr // Последним должен стать тот, что последний в отстортированном списке
            buff = Map()
            r
          } else {
            val r = sorted
  ...
  val followersFlow: Flow[Event, (Event, Followers), NotUsed] =
    Flow[Event].statefulMapConcat { () =>
      var followers: Map[Int, Set[Int]] = Map()
      ...
      event =>
        {
          event match {
            ...
            case Event.Follow(sequenceNr, fromUserId, toUserId) =>
              val newSet
              : Set[Int] = followers.getOrElse(fromUserId, Set()) + toUserId
              followers += (fromUserId -> newSet)
              event -> followers :: Nil

            case Event.Unfollow(sequenceNr, fromUserId, toUserId) =>
              followers = followers.map { t =>
                if (t._1 == fromUserId) {
                  t._1 -> t._2.filterNot(f => f == toUserId)
                } else t
              }
              event -> followers :: Nil
  ...
  def isNotified(userId: Int)(eventAndFollowers: (Event, Followers)): Boolean =
    eventAndFollowers._1 match {
      ...
      case Event.Unfollow(sequenceNr, fromUserId, toUserId) => userId == toUserId

      case Event.PrivateMsg(sequenceNr, fromUserId, toUserId) => userId == toUserId

      case Event.Follow(sequenceNr, fromUserId, toUserId) => userId == toUserId

      case Event.Broadcast(sequenceNr) => true

      // Что здесь происходит
      // Они перепутали имена переменных или тесты?
      // Нужно проверить, отправлять ли fromUserId изменения статуса userId, хотя казалось бы - источник это fromUserId
      case Event.StatusUpdate(sequenceNr, fromUserId) =>
        val userIdFollowers = eventAndFollowers._2.getOrElse(userId, Set())
        userIdFollowers.contains(fromUserId)
...
class Server()(implicit executionContext: ExecutionContext,
  ...
  val (inboundSink, broadcastOut) = {
    ...
    val hub
      : (Sink[ByteString, NotUsed], Source[(Event, model.Followers), NotUsed]) =
      MergeHub
  ...
  /**
    ...
    */
  //  broadcastOut: Source[(Event, model.Followers), NotUsed]
  //   val (inboundSink, broadcastOut) = {
  def outgoingFlow(userId: Int): Source[ByteString, NotUsed] =
    broadcastOut
      .filter(t => isNotified(userId)(t))
      .map(t => t._1.render)
...
//    clientIdPromise.future.map(id => actorSystem.log.info("Connected follower: {}", id.userId))
    ...
    val incoming: Sink[ByteString, NotUsed] =
      Flow[ByteString]
        .via(reframedFlow)
        .map(Identity.parse)
        .map(i => clientIdPromise.success(i))
        .to(Sink.ignore)
