...
trait Huffman extends HuffmanInterface {
  ...
  // количество каждого символа в массиве
  ...
  // по возрастанию, нужен именно тип List[Leaf] для возврата в makeOrderedLeafList
  ...
  /*
  Теперь я понял: combine должна делать один шаг и всё, а не итерировать по деревьям
  Поэтому ниже мы останавливаемся на x :: y :: Nil - тест был рассчитан на это

  def iter(trees: List[CodeTree]): List[CodeTree] =
    trees match {
    //case x :: Nil => trees
    /* Не очень понял, почему нужно остановиться на двух элементах, а не одном */
    case x :: y :: Nil => trees
    case x :: y :: xs => iter(sortCodeTree(Fork(x, y, chars(x) ::: chars(y), weight(x) + weight(y)) :: xs))
  }
  iter(trees)
  */
   ...
   */
  // если ноль - налево, если один - направо
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    // написал два варианта итераций
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = text match {
    case x :: Nil => tree match {
      ...
      case Fork(l, r, c, _) =>
        ...
        else if (chars(r).contains(x)) 1 :: encode(l)(x :: Nil)
